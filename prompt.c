/* prompt.c */
/* Input prompt and validation function implementations */
...
int parse_bitsm(char* str, char* err, void* netw, void* nBits)
{
	...
	/* Try to parse other input as a number of borrowed bits */
	else return parse_bb(str, err, netw, nBits);
}
...
int parse_max_subnum(char* str, char* err, void* netw, void* nBits)
{
	...
	if (str != end)
	{
		...
		if (!pass)
		{
			strcpy(err, "Invalid number of soobnets. ");
			return FALSE;
		}
		else return TRUE;
	}
...
int parse_max_hostnum(char* str, char* err, void* netw, void* nBits)
{
	...
	if (str != end)
	{
		...
		if (!pass)
		{
			strcpy(err, "Invalid number of hosts/soobnet. ");
			return FALSE;
		}
		else return TRUE;
	}
